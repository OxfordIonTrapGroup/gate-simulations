...
times, end_pop_upup, end_pop_dndn, end_pop_updn, end_pop_dnup, fidelities = ms_1.timescan(
        T=T_0,nT=100,delta_LS=delta_LS_0,ampl_asym=ampl_asym_1,ampl_asym_2=ampl_asym_1,ndot=0,
        species_Rabi_asym_MS=species_Rabi_asym_MS,Omega_R_2=Omega_R_2)
...
# fig, ax = plt.subplots()
# ax.plot(times/1e-6, np.array(end_pop_upup)+np.array(end_pop_updn),label='p11+p10')
# plt.grid(True,which="both",ls=":", color='0.7')
# plt.locator_params(axis='y', nbins=5)
# plt.locator_params(axis='x', nbins=5)
# plt.legend(loc="upper left", numpoints=1)
...
end_pop_upup, end_pop_dndn, end_pop_updn_dnup, fidelity = ms_1.parity_scan(phi_vec,
        delta_LS=delta_LS_0,ampl_asym=ampl_asym_1,
        scramble_mw_phase=False,fixed_analysis_phase=True,scramble_laser_phase=False)
# faster with scramble_mw_phase=False
    
fig1, ax1 = plt.subplots()
ax1.plot(phi_vec, np.real(end_pop_upup),label='p11')
ax1.plot(phi_vec, np.real(end_pop_updn_dnup),label='p10_01')
ax1.plot(phi_vec, np.real(end_pop_dndn),label='p00')
